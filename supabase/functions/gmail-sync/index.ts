...
serve(async (req) => {
  ...
  try {
    ...
    switch (action) {
      ...
      case 'start_sync': {
        // Minimal placeholder: mark running then paused and emit console lines.
        await setSyncStatus(supabase, userId, { sync_status: 'running', started_at: ts(), error_message: null });
        await sendConsole(supabase, userId, crypto.randomUUID(), 'Starting Gmail sync...', 'milestone', 'gmail');
        // TODO: implement actual sync. For now, immediately pause.
        await sendConsole(supabase, userId, null, 'Auto-paused (placeholder sync).', 'milestone', 'gmail');
