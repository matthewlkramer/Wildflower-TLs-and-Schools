/**
 * School "Linked" tab. Shows linked emails and meetings from Google
 * integration. Displays emails from z_g_emails table and meetings from
 * z_g_events table, both filtered by school_id.
 */
...
export function LinkedTab({ schoolId }: { schoolId?: string }) {
  ...
  const { data: emails = [], isLoading: emailsLoading } = useQuery<any[]>({
    queryKey: ["supabase/z_g_emails/school", schoolId],
    enabled: !!schoolId,
    queryFn: async () => {
  ...
  const { data: events = [], isLoading: eventsLoading } = useQuery<any[]>({
    queryKey: ["supabase/z_g_events/school", schoolId],
    enabled: !!schoolId,
    queryFn: async () => {
      const { data, error } = await supabase
        .from('z_g_events')
