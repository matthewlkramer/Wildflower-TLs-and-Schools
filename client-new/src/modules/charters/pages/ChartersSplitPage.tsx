...
import { useGridCharters, useCharterDetails } from '../api/queries';
...
export function ChartersSplitPage() {
  ...
  // Get details for selected charter
  const { data: selectedCharterData } = useCharterDetails(selectedCharterId || '');

  // Auto-select first charter if none selected
  React.useEffect(() => {
    if (data.length > 0 && !selectedCharterId) {
      setSelectedCharterId(String(data[0].id));
    }
  }, [data, selectedCharterId]);
  ...
  const cols = useMemo<ColDef<any>[]>(() => {
    ...
    for (const config of CHARTER_GRID) {
      ...
      const nameField = CHARTER_GRID.find(c => c.sortKey)?.field || 'charter_name';
        ...
        <div style={{ width: '320px', flexShrink: 0 }}>
          ...
          <GridBase
            ...
            gridOptions={{
              ...
              onRowClicked: (e) => {
                setSelectedCharterId(String(e.data.id));
              },
              onSelectionChanged: (e) => {
                const selectedRows = e.api.getSelectedRows();
                if (selectedRows.length > 0) {
                  setSelectedCharterId(String(selectedRows[0].id));
                }
              },
              rowSelection: {
                mode: 'singleRow',
